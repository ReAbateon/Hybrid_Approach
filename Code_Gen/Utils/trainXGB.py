...
def regressor_stats(joblib_path, X_q_test, y_q_test):
...
def model_stats(joblib_path, X_q_test, y_q_test):
    model = load(joblib_path)
...
def training(csv_path, n_trees, random_seed, test_size):
    ...
    if task == 0:
        if joblib_exists == 1:
            ...
            acc = model_stats(joblib_path, X_q_test, y_q_test)
            return test_path, joblib_path, sample_size, acc, csv_stem, n_classes, task , mae, mse, r2

        if(n_classes > 2):
            xgb = XGBClassifier(objective="multi:softprob", n_estimators=n_trees, num_class=n_classes, random_state=random_seed, n_jobs=-1, tree_method="hist")
        else:
            xgb = XGBClassifier(objective="binary:logistic", n_estimators=n_trees, random_state=random_seed, n_jobs=-1, tree_method="hist")
        ...
        if n_classes > 2:
                ...
                learning_rate=grid_search.best_params_["learning_rate"],    # shrinkage
                max_depth=grid_search.best_params_["max_depth"],          # profondità massima degli alberi
                subsample=grid_search.best_params_["subsample"],        # per regolarizzare
        ...
        return test_path, joblib_path, sample_size, accuracy, csv_stem, n_classes, task , mae, mse, r2
        ...
        if joblib_exists == 1:
            print("A Joblib already exists!")
            mae, mse, r2 = regressor_stats(joblib_path, X_q_test, y_q_test)
            return test_path, joblib_path, sample_size, acc, csv_stem, n_classes, task , mae, mse, r2


        xgb = XGBRegressor(objective="reg:squarederror", n_estimators=n_trees, random_state=random_seed, n_jobs=-1, tree_method="hist")

        param_grid = {
            "learning_rate":   [0.03, 0.05],
            "max_depth":       [3, 4, 5],
            "min_child_weight":[1, 5, 10],      # IMPORTANTISSIMO (controlla overfitting)
            "gamma":           [0, 0.1, 0.2, 0.5],
            "subsample":       [0.6, 0.8, 1.0],
            "colsample_bytree":[0.6, 0.8, 1.0], # IMPORTANTISSIMO
            "reg_alpha":       [0, 0.1, 0.2],
            "reg_lambda":      [1, 2, 3],
        }
