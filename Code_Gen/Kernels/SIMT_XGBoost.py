...
def generate_header(
    groups_per_class, n_classes, sample_size, parallelism, test_rows, maxv, minv
):
    return f"""\
#ifndef INC_SIMT_H_
#define INC_SIMT_H_
...
def set_model(num_class, acc, path, task, mae, mse, r2):
    ...
    if num_class > 2:
        ...
        for i in range(len(n_nodes)):
            ...
            size_bytes += n_nodes[i] * 8  # 8 bytes per node for SIMT
...
def SIMT_XGBoost_gen(
    ...
    with open(txt_name, "w") as f:
        for i in range(0, len(forest_levels), num_group):
            ...
            section = (
                "DTCM"
                if (groups_hybrid == -1 or totalgroupindex < groups_hybrid)
                else "RAM_BIG"
            )
            suffix = (
                "_dtcm"
            ...
            fh.write(
                f"{section} uint16_t feature{suffix}_{classindex}_{groupindex}[{len(feature_list)}] = {{{feature_str}}};\n"
            )
            fh.write(
                f"{section} int16_t threshold{suffix}_{classindex}_{groupindex}[{len(threshold_list)}] = {{{threshold_str}}};\n"
            )
            fh.write(
                f"{section} uint16_t childL{suffix}_{classindex}_{groupindex}[{len(childL_list)}] = {{{childL_str}}};\n"
            )
            fh.write(
                f"{section} uint16_t childR{suffix}_{classindex}_{groupindex}[{len(childR_list)}] = {{{childR_str}}};\n\n"
            )
...
def SIMT_XGBoost_gen_binary(
    thresholds, features, childL, childR, num_group, txt_name, groups_hybrid, fh
):
    forest_levels = []
    ...
    with open(txt_name, "w") as f:
        for i in range(0, len(forest_levels), num_group):
            ...
            section = (
                "DTCM"
                if (groups_hybrid == -1 or groupindex < groups_hybrid)
                else "RAM_BIG"
            )
            suffix = (
                "_dtcm" if (groups_hybrid == -1 or groupindex < groups_hybrid) else ""
            )

            fh.write(
                f"{section} uint16_t feature{suffix}{groupindex}[{len(feature_list)}] = {{{feature_str}}};\n"
            )
            fh.write(
                f"{section} int16_t threshold{suffix}{groupindex}[{len(threshold_list)}] = {{{threshold_str}}};\n"
            )
            fh.write(
                f"{section} uint16_t childL{suffix}{groupindex}[{len(childL_list)}] = {{{childL_str}}};\n"
            )
            fh.write(
                f"{section} uint16_t childR{suffix}{groupindex}[{len(childR_list)}] = {{{childR_str}}};\n\n"
            )

            groupindex += 1


def generate_inference(fh, n_classes, groups_per_class, groups_hybrid):
    ...
    for j in range(n_classes):
        for i in range(groups_per_class):
            suffix = (
                "_dtcm" if (groups_hybrid == -1 or total_groups < groups_hybrid) else ""
            )
            line = f"   myclass = kernel_simt(threshold{suffix}_{j}_{i}, feature{suffix}_{j}_{i}, childL{suffix}_{j}_{i}, childR{suffix}_{j}_{i}, sample);\n"
            lines.append(line)
...
def generate_inference_binary(fh, groups, groups_hybrid):
    fh.write("static inline void inference(int16_t* sample){\n")
    fh.write("   uint8_t i = 0;\n")
    fh.write("   classes myclass;\n")
    ...
    for i in range(groups):
        suffix = "_dtcm" if (groups_hybrid == -1 or i < groups_hybrid) else ""
        line = f"   myclass = kernel_simt(threshold{suffix}{i}, feature{suffix}{i}, childL{suffix}{i}, childR{suffix}{i}, sample);\n"
        lines.append(line)
    ...
    if n_classes > 2:
        ...
        if hybrid != -1:
            ...
            with open(header_path, "w") as f:
                ...
                f.write(kernel)
                generate_inference(f, n_classes, groups_per_class, groups_hybrid)
            ...
            with open(header_path, "w") as f:
                ...
                f.write(kernel)
                generate_inference(f, n_classes, groups_per_class, -1)
                testset_gen(test_path, f, number_of_test_samples)
                f.write("#endif\n")
        ...
        if hybrid != -1:
            ...
            print(f"Only {hybrid} trees / {groups_hybrid} groups will fit in DTCM!")
            header_name = (
                f"{csv_stem}_XGB_simt_hybrid_G{groups}_D{max_depth}_RS{random_seed}.h"
            )
            txt_name = (
                f"{csv_stem}_XGB_simt_hybrid_G{groups}_D{max_depth}_RS{random_seed}.txt"
            )
            ...
            header = generate_header_binary(
                groups, sample_size, parallelism, number_of_test_samples, maxv, minv
            )
            kernel = generate_kernel()

            with open(header_path, "w") as f:
                f.write(header)
                SIMT_XGBoost_gen_binary(
                    thresholds,
                    features,
                    childL,
                    childR,
                ...
                f.write(kernel)
                generate_inference_binary(f, groups, groups_hybrid)
                testset_gen(test_path, f, number_of_test_samples)
                f.write("#endif\n")
        else:
            print("The model fits entirely in DTCM!")
            header_name = (
                f"{csv_stem}_XGB_simt_G{groups}_D{max_depth}_RS{random_seed}.h"
            )
            ...
            with open(header_path, "w") as f:
                f.write(header)
                SIMT_XGBoost_gen_binary(
                    thresholds, features, childL, childR, parallelism, txt_path, -1, f
                )
                f.write(kernel)

                generate_inference_binary(f, groups, -1)
                testset_gen(test_path, f, number_of_test_samples)
                f.write("#endif\n")
